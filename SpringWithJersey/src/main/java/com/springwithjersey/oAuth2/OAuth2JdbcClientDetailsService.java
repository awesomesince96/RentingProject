...
public class OAuth2JdbcClientDetailsService implements ClientDetailsService, ClientRegistrationService {
  /** The Constant logger. */
  private static final Log logger = LogFactory.getLog(OAuth2JdbcClientDetailsService.class);

  /** The Constant CLIENT_FIELDS_FOR_UPDATE. */
  private static final String CLIENT_FIELDS_FOR_UPDATE =
      "ResourceIds, Scope, " + "AuthorizedGrantTypes, WebServerRedirectUri, Authorities, AccessTokenValidity, "
  ...
  private static final String BASE_FIND_STATEMENT =
      "select ClientId, " + CLIENT_FIELDS + " from oauth2clientdetails";

  /** The Constant DEFAULT_FIND_STATEMENT. */
  private static final String DEFAULT_FIND_STATEMENT = BASE_FIND_STATEMENT + " order by ClientId";

  /** The Constant DEFAULT_SELECT_STATEMENT. */
  private static final String DEFAULT_SELECT_STATEMENT = BASE_FIND_STATEMENT + " where ClientId = ?";

  /** The Constant DEFAULT_INSERT_STATEMENT. */
  private static final String DEFAULT_INSERT_STATEMENT =
      "insert into oauth2clientdetails (" + CLIENT_FIELDS + ", ClientId) values (?,?,?,?,?,?,?,?,?,?,?)";

  /** The Constant DEFAULT_UPDATE_STATEMENT. */
  private static final String DEFAULT_UPDATE_STATEMENT = "update oauth2clientdetails " + "set "
  ...
  private static final String DEFAULT_UPDATE_SECRET_STATEMENT =
      "update oauth2clientdetails" + "set ClientSecret = ? where ClientId = ?";

  /** The Constant DEFAULT_DELETE_STATEMENT. */
  private static final String DEFAULT_DELETE_STATEMENT =
      "delete from oauth2clientdetails where ClientId = ?";
  ...
  /**
   * Instantiates a new mob gate jdbc client details service.
   *
   * @param dataSource the data source
   */
  public OAuth2JdbcClientDetailsService(DataSource dataSource) {
   ...
   * (non-Javadoc)
   * 
   * @see org.springframework.security.oauth2.provider.ClientRegistrationService#addClientDetails(org.springframework.
   * security .oauth2.provider.ClientDetails)
   */
   ...
   * 
   * @see
   * org.springframework.security.oauth2.provider.ClientRegistrationService#updateClientDetails(org.springframework.
   * security.oauth2.provider.ClientDetails)
   */
   ...
   * (non-Javadoc)
   * 
   * @see org.springframework.security.oauth2.provider.ClientRegistrationService#updateClientSecret(java.lang.String,
   * java.lang.String)
   */
  ...
  private Object[] getFieldsForUpdate(ClientDetails clientDetails) {
    String json = null;
    try {
      json = objectMapper.writeValueAsString(clientDetails.getAdditionalInformation());
    } catch (Exception e) {
    ...
    return new Object[] {
        ...
        clientDetails.getScope() != null ? StringUtils.collectionToCommaDelimitedString(clientDetails.getScope())
            : null,
      ...
      if (!StringUtils.isEmpty(json)) {
        try {
          ...
          Map<String, Object> additionalInformation =
              OAuth2JdbcClientDetailsService.this.objectMapper.readValue(json, Map.class);
